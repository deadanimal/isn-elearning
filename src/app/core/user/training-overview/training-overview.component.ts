...
export class TrainingOverviewComponent implements OnInit {
   ...
   getData() {
     this.mockService.getAll('user-training/training.data.json').subscribe(
       (res) => {
         // Success
         this.tableRows = [...res]
   ...
   confirm() {
     swal.fire({
       title: "Confirmation",
       text: "Are you sure to apply for this training?",
       type: "info",
   ...
   register() {
     swal.fire({
       title: "Success",
       text: "You have been admitted to this training!",
       type: "success",
       buttonsStyling: false,
